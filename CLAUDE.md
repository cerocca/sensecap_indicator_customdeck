...
18. **`sensedeck_proxy.py` — auto-restart**: `_record_error()` chiama `os._exit(42)` al 10° errore consecutivo. `SenseDeck_Proxy_Start.command` lancia un wrapper `bash` con loop: se il processo Python esce con codice 42 il wrapper attende 3s, poi esegue un ping loop su `192.168.1.69` (Sibilla) finché non risponde, quindi rilancia Python; qualsiasi altro codice ferma il loop. Non usare `os.execv`: non funziona sotto `nohup` (rimane in stato broken). Il ping loop è necessario perché i socket corrotti vengono ereditati dal nuovo processo Python se Sibilla non è ancora raggiungibile al momento del restart — il Stop/Start manuale funziona perché uccide anche il wrapper e libera tutto.
