...
/* forward declarations — schermate custom, usate nei gesture handler Seeed */
extern lv_obj_t *ui_screen_hue;
extern lv_obj_t *ui_screen_ai;
...
if ( event_code == LV_EVENT_GESTURE &&  lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_RIGHT  ) {
      _ui_screen_change( ui_screen_ai, LV_SCR_LOAD_ANIM_MOVE_RIGHT, 200, 0);  /* ← chiude il cerchio */
}
...
if ( event_code == LV_EVENT_GESTURE &&  lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_LEFT  ) {
      _ui_screen_change( ui_screen_settings_custom, LV_SCR_LOAD_ANIM_MOVE_LEFT, 200, 0);  /* → settings custom */
}
...
void ui_event_screen_setting( lv_event_t * e) {
    lv_event_code_t event_code = lv_event_get_code(e);lv_obj_t * target = lv_event_get_target(e);
if ( event_code == LV_EVENT_GESTURE &&  lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_LEFT  ) {
      _ui_screen_change( ui_screen_hue, LV_SCR_LOAD_ANIM_MOVE_LEFT, 200, 0);  /* → custom screens */
}
if ( event_code == LV_EVENT_GESTURE &&  lv_indev_get_gesture_dir(lv_indev_get_act()) == LV_DIR_RIGHT  ) {
